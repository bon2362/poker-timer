...
describe('SettingsScreen — Header and navigation', () => {
  ...
  test('renders version "v4.28"', () => {
    renderSettings();
    expect(screen.getByText('v4.28')).toBeInTheDocument();
  });
...
describe('SettingsScreen — Changelog', () => {
  test('clicking version "v4.28" opens changelog modal', async () => {
    const user = userEvent.setup();
    renderSettings();
    await user.click(screen.getByText('v4.28'));
