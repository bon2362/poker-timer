...
describe('SettingsScreen — Header and navigation', () => {
  ...
  test('renders version "v4.24"', () => {
    renderSettings();
    expect(screen.getByText('v4.24')).toBeInTheDocument();
  });
...
describe('SettingsScreen — Changelog', () => {
  test('clicking version "v4.24" opens changelog modal', async () => {
    const user = userEvent.setup();
    renderSettings();
    await user.click(screen.getByText('v4.24'));
