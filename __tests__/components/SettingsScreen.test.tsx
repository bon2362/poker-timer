...
describe('SettingsScreen — Header and navigation', () => {
  ...
  test('renders version "v4.27"', () => {
    renderSettings();
    expect(screen.getByText('v4.27')).toBeInTheDocument();
  });
...
describe('SettingsScreen — Changelog', () => {
  test('clicking version "v4.27" opens changelog modal', async () => {
    const user = userEvent.setup();
    renderSettings();
    await user.click(screen.getByText('v4.27'));
